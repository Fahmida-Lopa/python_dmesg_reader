...
"""
@brief:     This function calls the dmesg_reader.py with the given
            file and count the occurence of error in the JSON output file
@input:     filename, name of the file
...
def getErrorCountFromTheReader(filename, until):
    cmd = "python3 dmesg_reader.py -i {0} -o output_test.json -u {1}".format(
        filename, until
    )
    os.system(cmd)
...
def main():
    ...
    test_log_error = getErrorCountFromTheReader("dmesg_error_test.log",
                                                "2023-03-08")
